...
class StatisticsInfoMapper:
    ...
    def select_statistics_info_list(cls, statistics_info: StatisticsInfo) -> List[StatisticsInfo]:
        ...
        try:
            ...
            stmt = cls.init_query(statistics_info, stmt)

            _params = getattr(statistics_info, "params", {}) or {}
    ...
    @classmethod
    def init_query(cls, statistics_info, stmt):
        if statistics_info.id is not None:
            stmt = stmt.where(StatisticsInfoPo.id == statistics_info.id)
        if statistics_info.type is not None:
            stmt = stmt.where(StatisticsInfoPo.type == statistics_info.type)
        if statistics_info.statistics_name:
            stmt = stmt.where(
                StatisticsInfoPo.statistics_name.like("%" + str(statistics_info.statistics_name) + "%"))
        if statistics_info.common_key:
            stmt = stmt.where(StatisticsInfoPo.common_key.like("%" + str(statistics_info.common_key) + "%"))
        if statistics_info.statistics_key:
            # 【性能优化】使用精确匹配而不是模糊查询，否则无法使用索引
            stmt = stmt.where(StatisticsInfoPo.statistics_key.like("%" + str(statistics_info.statistics_key) + "%"))
            # stmt = stmt.where(StatisticsInfoPo.statistics_key == statistics_info.statistics_key)
        if statistics_info.content is not None:
            stmt = stmt.where(StatisticsInfoPo.content == statistics_info.content)
        if statistics_info.extend_content is not None:
            stmt = stmt.where(StatisticsInfoPo.extend_content == statistics_info.extend_content)
        if statistics_info.remark is not None:
            stmt = stmt.where(StatisticsInfoPo.remark == statistics_info.remark)
        return stmt
    ...
    def insert_statistics_info(cls, statistics_info: StatisticsInfo) -> int:
        ...
        try:
            ...
            new_po.type = statistics_info.type
            new_po.statistics_name = statistics_info.statistics_name
            new_po.common_key = statistics_info.common_key
            new_po.statistics_key = statistics_info.statistics_key
            new_po.content = statistics_info.content
            new_po.extend_content = statistics_info.extend_content
            new_po.remark = statistics_info.remark
            new_po.create_time = statistics_info.create_time or now
            db.session.add(new_po)
            db.session.commit()
            statistics_info.id = new_po.id
            return 1
        except Exception as e:
            db.session.rollback()
            print(f"新增统计信息出错: {e}")
    ...
    def update_statistics_info(cls, statistics_info: StatisticsInfo) -> int:
        ...
        try:
            ...
            existing = db.session.get(StatisticsInfoPo, statistics_info.id)
            if not existing:
                return 0
            now = datetime.now()
            # 主键不参与更新
            existing.type = statistics_info.type
            existing.statistics_name = statistics_info.statistics_name
            existing.common_key = statistics_info.common_key
            existing.statistics_key = statistics_info.statistics_key
